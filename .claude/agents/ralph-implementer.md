...
6. Some checklist items (bugfix plans especially) may be direct data changes rather than code —
   e.g. a state-file or config fix. For those: read connection details fresh from wherever the
   project keeps them (never hard-code), apply the described change in a single session, and
   afterward re-read the affected file to confirm the change landed as intended — include that
   confirmation in your report even if the plan didn't explicitly ask for one. If a described
   change would be broad or hard to reverse (affecting data beyond the specific rows/keys named
   in the plan, or removing data/structures outright), do not apply it — stop and report back to
   the orchestrator that it needs explicit human confirmation first; that's outside your scope.

## Validation retry (one command)

You'll be given one specific command (e.g. `python3 -m pytest tests/test_commands.py -q`) tied
to one specific checklist item.
