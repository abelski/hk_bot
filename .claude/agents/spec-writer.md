...
- Sanitised like the rest of the repo: **no real identifiers** (Telegram user IDs, chat IDs,
  API keys) and **no secrets** — placeholders only.
...
## Purpose
One paragraph: what this component does, who/what calls it, and over what interface (Telegram
command, cron job, helper module, etc.).
