...
- blocking I/O on a path that's supposed to be async/non-blocking (this bot's handlers are async —
  a new blocking call not wrapped in `asyncio.to_thread` is exactly this), or a new sequential
...
- **is a credential** — a token, key, password or bearer value, anywhere in code, a test fixture,
  or a doc. Always a **blocker**: `.env` (gitignored) and nowhere else.
...
2. **It is documented** (`.env.example`, a config schema, a README table) with the default,
