...
it works identically no matter which skill produced the plan (`feature_analyst` or anything else
...
- Never commit or push to git — this skill (and the subagents it spawns) never does, regardless
  ...
  frontmatter, not git history. This project's `.claude/hooks/block-git-push.sh` also blocks any
  `git push` structurally, as a backstop.
...
- This skill has zero knowledge of caller-specific epilogues (publishing, notifications, moving
  ...
  skill, not here.
- A code-review gate between Step 4 and Step 5 is available: `.claude/agents/ralph-reviewer.md`
  in this project — a read-only reviewer subagent over the diff before validation runs. Not
  wired in by default to keep the loop's cost predictable for small changes; add a step here if
  this project's history justifies the extra round trip.
